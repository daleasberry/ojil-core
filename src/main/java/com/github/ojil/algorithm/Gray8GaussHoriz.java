...
/**
 * Computes a horizontal Gaussian blur for an input gray image. The sigma value
...
public class Gray8GaussHoriz extends PipelineStage {
     ...
     * so we just give one side below.)
     */
    /**
     * The array below was generated using two Excel macros: In cell b2 and
     * filled out to column AM and down:
     * =EXP(-(COLUMN(B2)-1)*(COLUMN(B2)-1)/((ROW(B2)-1)/10*(ROW(B2)-1)/10)) To
     * compute the value shown below: =ROUND(B2/SUM($B2:$AM2)*256,0)
     */
     ...
     *            integer.
     * @throws ImageError
     *             is sigma is out of range.
     */
    public Gray8GaussHoriz(final int cSigma) throws ImageError {
        setSigma(cSigma);
    }
    
    /**
     * Apply the Gaussian horizontally to the input input image. The sigma value
     * for the operator is set in the class constructor. We handle the borders
     * of the image a little carefully to avoid creating spurious edges at them.
     * The image value at the border is reflected so that image(0,-1), for
     * example, is made equal to image(0,1).
     *
     * @param image
     ...
     * the operator responds to. A small sigma value gives less blur.
     *
     * @param cSigma
     *            the new sigma value
     * @throws ImageError
     *             if cSigma is out of range -- less than or equal to 1 or
     *             greater than the number of coefficients we're precomputed.
     */
