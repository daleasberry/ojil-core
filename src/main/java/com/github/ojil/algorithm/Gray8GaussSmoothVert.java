...
public class Gray8GaussSmoothVert extends PipelineStage {
     ...
     */
    public Gray8GaussSmoothVert(final int cSigma) throws ImageError {
        setSigma(cSigma);
    }
     ...
     * The image value at the border is reflected so that image(-1,0), for
     * example, is made equal to image(1,0).
     *
     * @param image
            ...
            for (int i = 0; i < cHeight; i++) {
                ...
                for (int k = 0; k < wCoeff.length; k++) {
                    if ((i + k) < cHeight) {
                        ...
                        final int cPos = (2 * cHeight) - (i + k + 1);
